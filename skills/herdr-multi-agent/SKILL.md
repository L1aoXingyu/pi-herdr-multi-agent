...
whatever `herdr agent --kind` starts (default fleet: dsh-TUI + Pi + Cursor).
...
`--fleet-file`). They are **examples** — every model id must already work in the caller's Grok/cursor/claude/codex/dsh config. This parent is Grok Build; do not require a Pi model list.
